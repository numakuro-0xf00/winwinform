...
### 1. [Critical] `wfth-correlate` の CLI 契約が文書間で衝突している
...
### 2. [High] 出力フォーマット（`session.ndjson` vs `session.json`）が不一致
...
### 3. [High] ノイズ分類の担当（aggregate / correlate）が衝突している
...
### 4. [High] Spec Parser の入力拡張子定義が自己矛盾している（`.xls`）
...
### 6. [Medium] フック生存監視ロジックに誤検知リスクがある
...
### 8. [Medium] 最上位の全体設計書が最新分割方針を反映していない
...
1. `wfth-correlate` の入出力契約を確定し、関連ドキュメントを同日更新する  
2. ノイズ分類の責務を aggregate / correlate のどちらかに固定する  
3. Spec Parser の入力仕様を `.xlsx` に統一し、`.xls` の扱いを明文化する  
4. IPC の接続保護（命名、ACL、ハンドシェイク）を最小限で設計に追加する  
5. Hook 監視ロジックの誤検知条件とキュー劣化時の挙動を定義する
